...
export default ({ mode }) => {
  const blogApiTarget =
    process.env.VITE_BLOG_API_TARGET ||
    (process.env.VITE_BLOG_API_PORT
  ...
  return defineConfig({
      ...
      VitePWA({
        ...
        workbox: {
          ...
          runtimeCaching: [
            ...
            {
              urlPattern:
                /^(?!.*\/(mgmt|blog|api|music|static)\/).*(.*?)\.(png|jpe?g|svg|gif|ico|webp)$/,
              handler: 'CacheFirst',
              options: {
    ...
    server: {
      ...
      proxy: {
        ...
        // 代理博客 API；普通开发入口使用生产公开 API，本地入口使用 8086
        '/api': {
          target: blogApiTarget,
          changeOrigin: true,
        },
        // 配置接口返回的头像、二维码使用 /uploads 相对路径
        '/uploads': {
          target: blogApiTarget,
          changeOrigin: true,
        },
