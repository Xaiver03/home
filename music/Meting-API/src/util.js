...
export const get_url = (ctx) => {
  ...
  // 检测 HTTPS：通过 X-Forwarded-Proto 头或运行环境
  const isHTTPS = ctx.req.header('X-Forwarded-Proto') === 'https' ||
                  runtime === 'vercel' ||
                  ctx.req.header('X-Forwarded-Ssl') === 'on'

  if (isHTTPS) req_url = req_url.replace('http://', 'https://')

  // 特殊处理：如果通过Nginx代理访问，使用正确的外部路径
  if (ctx.req.header('X-Forwarded-Url')) {
    return ctx.req.header('X-Forwarded-Url')
  }
